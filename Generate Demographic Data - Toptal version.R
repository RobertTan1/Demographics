# Run to install necessary packages
install.packages(
  c(
    "randomNames",
    "tidyverse",
...
library(msm)          # For generating data
...
library(curl)         # Parallelize api calls

# Needs google api key with google maps geocode api to be turned on
...
customers <-
  data.frame(
    name = randomNames(50, name.order = "first.last", name.sep = " "),
    email = NA
  )
